export const packagesMenuDatas = [
  { id: 1, img: "/images/packagesitems/3.jpg", name: "Countryside", nameId: "countryside" },
  { id: 2, img: "/images/packagesitems/4.jpg", name: "Icons", nameId: "icons" },
  { id: 3, img: "/images/packagesitems/5.jpg", name: "Islands", nameId: "islands" },
  { id: 4, img: "/images/packagesitems/6.jpg", name: "Castles", nameId: "castles" },
  { id: 5, img: "/images/packagesitems/7.jpg", name: "Rooms", nameId: "rooms" },
  { id: 6, img: "/images/packagesitems/8.jpg", name: "Cabins", nameId: "cabins" },
  { id: 7, img: "/images/packagesitems/5.jpg", name: "topoftheworld", nameId: "topoftheworld" },
  { id: 8, img: "/images/packagesitems/9.jpg", name: "Amazing Views", nameId: "amazingviews" },
  { id: 9, img: "/images/packagesitems/10.jpg", name: "Farms", nameId: "farms" },
  { id: 10, img: "/images/packagesitems/11.jpg", name: "Amazing Pools", nameId: "amazingpools" },
  { id: 11, img: "/images/packagesitems/13.jpg", name: "Beachfront", nameId: "beachfront" },
  { id: 12, img: "/images/packagesitems/14.jpg", name: "OMG!", nameId: "omg" },
  { id: 13, img: "/images/packagesitems/4.jpg", name: "Luxe", nameId: "luxe" },
  { id: 14, img: "/images/packagesitems/5.jpg", name: "Treehouses", nameId: "treehouses" },
  { id: 15, img: "/images/packagesitems/6.jpg", name: "Lakefront", nameId: "lakefront" },
  { id: 16, img: "/images/packagesitems/7.jpg", name: "Top of the Worldsdsdsd", nameId: "topoftheworldsss" },
  { id: 17, img: "/images/packagesitems/8.jpg", name: "National Parks", nameId: "nationalparks" },
  { id: 18, img: "/images/packagesitems/9.jpg", name: "Historical Homes", nameId: "historicalhomes" },
  { id: 19, img: "/images/packagesitems/10.jpg", name: "Tiny Homes", nameId: "tinyhomes" },
  { id: 20, img: "/images/packagesitems/11.jpg", name: "Mansions", nameId: "mansions" },
  { id: 21, img: "/images/packagesitems/13.jpg", name: "Domes", nameId: "domes" },
  { id: 22, img: "/images/packagesitems/14.jpg", name: "Design", nameId: "design" },
];

export const sectonTowPlaces = {
  countryside: [
    {
      id: 1,
      location: "Chennai, India",
      rating: 5.0,
      distance: "542 Kilometres away",
      date: "1-6 Mar",
      price: "₹21,979",
      guestFavorite: true,
      checkinDate: "2025-03-01",  // Demo Check-in Date
      checkoutDate: "2025-03-06", // Demo Check-out Date
      images: [
        "/images/packaging/6.jpg",
        "/images/packaging/7.jpg",
        "/images/packaging/8.jpg",
        "/images/packaging/9.jpg",
        "/images/packaging/10.jpg",
        "/images/packaging/11.jpg",
        "/images/packaging/13.jpg",
      ],
      description:
        "Experience the beauty of nature with a peaceful stay in Chennai’s countryside. Perfect for relaxing getaways!",
      amenities: ["Free WiFi", "Breakfast Included", "Swimming Pool", "Parking"],
      reviews: [
        { user: "Rahul", comment: "Amazing experience! The view was breathtaking." },
        { user: "Meera", comment: "Loved the hospitality and the food!" },
      ],
      highlights: [
        "Beautiful landscapes",
        "Local village tours",
        "Organic food options",
      ],
      detailsPageHeading: ["The Hill Tranquil Pool Villa@Nandi Hills"],


    },


    {
      id: 2,
      location: "demo, India",
      rating: 5.0,
      distance: "542 Kilometres away",
      date: "1-6 Mar",
      price: "₹21,979",
      guestFavorite: true,
      images: [
        "/images/packaging/9.jpg",
        "/images/packaging/7.jpg",
        "/images/packaging/8.jpg"
      ],
      description: "Experience the beauty of nature with a peaceful stay in Chennai’s countryside. Perfect for relaxing getaways!",
      amenities: ["Free WiFi", "Breakfast Included", "Swimming Pool", "Parking"],
      reviews: [
        { user: "Rahul", comment: "Amazing experience! The view was breathtaking." },
        { user: "Meera", comment: "Loved the hospitality and the food!" }
      ],
      highlights: ["Beautiful landscapes", "Local village tours", "Organic food options"]
    },
    {
      id: 3,
      location: "sadasda",
      rating: 5.0,
      distance: "542 Kilometres away",
      date: "1-6 Mar",
      price: "₹21,979",
      guestFavorite: true,
      images: [
        "/images/packaging/15.jpg",
        "/images/packaging/14.jpg",
        "/images/packaging/12.jpg"
      ],
      description: "Experience the beauty of nature with a peaceful stay in Chennai’s countryside. Perfect for relaxing getaways!",
      amenities: ["Free WiFi", "Breakfast Included", "Swimming Pool", "Parking"],
      reviews: [
        { user: "Rahul", comment: "Amazing experience! The view was breathtaking." },
        { user: "Meera", comment: "Loved the hospitality and the food!" }
      ],
      highlights: ["Beautiful landscapes", "Local village tours", "Organic food options"]
    },
    {
      id: 4,
      location: "demo, India",
      rating: 5.0,
      distance: "542 Kilometres away",
      date: "1-6 Mar",
      price: "₹21,979",
      guestFavorite: true,
      images: [
        "/images/packaging/9.jpg",
        "/images/packaging/7.jpg",
        "/images/packaging/8.jpg"
      ],
      description: "Experience the beauty of nature with a peaceful stay in Chennai’s countryside. Perfect for relaxing getaways!",
      amenities: ["Free WiFi", "Breakfast Included", "Swimming Pool", "Parking"],
      reviews: [
        { user: "Rahul", comment: "Amazing experience! The view was breathtaking." },
        { user: "Meera", comment: "Loved the hospitality and the food!" }
      ],
      highlights: ["Beautiful landscapes", "Local village tours", "Organic food options"]
    },
    {
      id: 5,
      location: "demo, India",
      rating: 5.0,
      distance: "542 Kilometres away",
      date: "1-6 Mar",
      price: "₹21,979",
      guestFavorite: true,
      images: [
        "/images/packaging/10.jpg",
        "/images/packaging/7.jpg",
        "/images/packaging/8.jpg"
      ],
      description: "Experience the beauty of nature with a peaceful stay in Chennai’s countryside. Perfect for relaxing getaways!",
      amenities: ["Free WiFi", "Breakfast Included", "Swimming Pool", "Parking"],
      reviews: [
        { user: "Rahul", comment: "Amazing experience! The view was breathtaking." },
        { user: "Meera", comment: "Loved the hospitality and the food!" }
      ],
      highlights: ["Beautiful landscapes", "Local village tours", "Organic food options"]
    },
    {
      id: 6,
      location: "demo, India",
      rating: 5.0,
      distance: "542 Kilometres away",
      date: "1-6 Mar",
      price: "₹21,979",
      guestFavorite: true,
      images: [
        "/images/packaging/11.jpg",
        "/images/packaging/7.jpg",
        "/images/packaging/8.jpg"
      ],
      description: "Experience the beauty of nature with a peaceful stay in Chennai’s countryside. Perfect for relaxing getaways!",
      amenities: ["Free WiFi", "Breakfast Included", "Swimming Pool", "Parking"],
      reviews: [
        { user: "Rahul", comment: "Amazing experience! The view was breathtaking." },
        { user: "Meera", comment: "Loved the hospitality and the food!" }
      ],
      highlights: ["Beautiful landscapes", "Local village tours", "Organic food options"]
    },
    {
      id: 7,
      location: "demo, India",
      rating: 5.0,
      distance: "542 Kilometres away",
      date: "1-6 Mar",
      price: "₹21,979",
      guestFavorite: true,
      images: [
        "/images/packaging/13.jpg",
        "/images/packaging/7.jpg",
        "/images/packaging/8.jpg"
      ],
      description: "Experience the beauty of nature with a peaceful stay in Chennai’s countryside. Perfect for relaxing getaways!",
      amenities: ["Free WiFi", "Breakfast Included", "Swimming Pool", "Parking"],
      reviews: [
        { user: "Rahul", comment: "Amazing experience! The view was breathtaking." },
        { user: "Meera", comment: "Loved the hospitality and the food!" }
      ],
      highlights: ["Beautiful landscapes", "Local village tours", "Organic food options"]
    },
    {
      id: 8,
      location: "demo, India",
      rating: 5.0,
      distance: "542 Kilometres away",
      date: "1-6 Mar",
      price: "₹21,979",
      guestFavorite: true,
      images: [
        "/images/packaging/14.jpg",
        "/images/packaging/7.jpg",
        "/images/packaging/8.jpg"
      ],
      description: "Experience the beauty of nature with a peaceful stay in Chennai’s countryside. Perfect for relaxing getaways!",
      amenities: ["Free WiFi", "Breakfast Included", "Swimming Pool", "Parking"],
      reviews: [
        { user: "Rahul", comment: "Amazing experience! The view was breathtaking." },
        { user: "Meera", comment: "Loved the hospitality and the food!" }
      ],
      highlights: ["Beautiful landscapes", "Local village tours", "Organic food options"]
    },
    {
      id: 9,
      location: "demo, India",
      rating: 5.0,
      distance: "542 Kilometres away",
      date: "1-6 Mar",
      price: "₹21,979",
      guestFavorite: true,
      images: [
        "/images/packaging/15.jpg",
        "/images/packaging/7.jpg",
        "/images/packaging/8.jpg"
      ],
      description: "Experience the beauty of nature with a peaceful stay in Chennai’s countryside. Perfect for relaxing getaways!",
      amenities: ["Free WiFi", "Breakfast Included", "Swimming Pool", "Parking"],
      reviews: [
        { user: "Rahul", comment: "Amazing experience! The view was breathtaking." },
        { user: "Meera", comment: "Loved the hospitality and the food!" }
      ],
      highlights: ["Beautiful landscapes", "Local village tours", "Organic food options"]
    },
    {
      id: 10,
      location: "demo, India",
      rating: 5.0,
      distance: "542 Kilometres away",
      date: "1-6 Mar",
      price: "₹21,979",
      guestFavorite: true,
      images: [
        "/images/packaging/6.jpg",
        "/images/packaging/7.jpg",
        "/images/packaging/8.jpg"
      ],
      description: "Experience the beauty of nature with a peaceful stay in Chennai’s countryside. Perfect for relaxing getaways!",
      amenities: ["Free WiFi", "Breakfast Included", "Swimming Pool", "Parking"],
      reviews: [
        { user: "Rahul", comment: "Amazing experience! The view was breathtaking." },
        { user: "Meera", comment: "Loved the hospitality and the food!" }
      ],
      highlights: ["Beautiful landscapes", "Local village tours", "Organic food options"]
    },
    {
      id: 11,
      location: "demo, India",
      rating: 5.0,
      distance: "542 Kilometres away",
      date: "1-6 Mar",
      price: "₹21,979",
      guestFavorite: true,
      images: [
        "/images/packaging/7.jpg",
        "/images/packaging/6.jpg",
        "/images/packaging/8.jpg"
      ],
      description: "Experience the beauty of nature with a peaceful stay in Chennai’s countryside. Perfect for relaxing getaways!",
      amenities: ["Free WiFi", "Breakfast Included", "Swimming Pool", "Parking"],
      reviews: [
        { user: "Rahul", comment: "Amazing experience! The view was breathtaking." },
        { user: "Meera", comment: "Loved the hospitality and the food!" }
      ],
      highlights: ["Beautiful landscapes", "Local village tours", "Organic food options"]
    },
    {
      id: 12,
      location: "demo, India",
      rating: 5.0,
      distance: "542 Kilometres away",
      date: "1-6 Mar",
      price: "₹21,979",
      guestFavorite: true,
      images: [
        "/images/packaging/8.jpg",
        "/images/packaging/7.jpg",
        "/images/packaging/8.jpg"
      ],
      description: "Experience the beauty of nature with a peaceful stay in Chennai’s countryside. Perfect for relaxing getaways!",
      amenities: ["Free WiFi", "Breakfast Included", "Swimming Pool", "Parking"],
      reviews: [
        { user: "Rahul", comment: "Amazing experience! The view was breathtaking." },
        { user: "Meera", comment: "Loved the hospitality and the food!" }
      ],
      highlights: ["Beautiful landscapes", "Local village tours", "Organic food options"]
    },
    {
      id: 13,
      location: "demo, India",
      rating: 5.0,
      distance: "542 Kilometres away",
      date: "1-6 Mar",
      price: "₹21,979",
      guestFavorite: true,
      images: [
        "/images/packaging/9.jpg",
        "/images/packaging/7.jpg",
        "/images/packaging/8.jpg"
      ],
      description: "Experience the beauty of nature with a peaceful stay in Chennai’s countryside. Perfect for relaxing getaways!",
      amenities: ["Free WiFi", "Breakfast Included", "Swimming Pool", "Parking"],
      reviews: [
        { user: "Rahul", comment: "Amazing experience! The view was breathtaking." },
        { user: "Meera", comment: "Loved the hospitality and the food!" }
      ],
      highlights: ["Beautiful landscapes", "Local village tours", "Organic food options"]
    },
    {
      id: 14,
      location: "demo, India",
      rating: 5.0,
      distance: "542 Kilometres away",
      date: "1-6 Mar",
      price: "₹21,979",
      guestFavorite: true,
      images: [
        "/images/packaging/10.jpg",
        "/images/packaging/7.jpg",
        "/images/packaging/8.jpg"
      ],
      description: "Experience the beauty of nature with a peaceful stay in Chennai’s countryside. Perfect for relaxing getaways!",
      amenities: ["Free WiFi", "Breakfast Included", "Swimming Pool", "Parking"],
      reviews: [
        { user: "Rahul", comment: "Amazing experience! The view was breathtaking." },
        { user: "Meera", comment: "Loved the hospitality and the food!" }
      ],
      highlights: ["Beautiful landscapes", "Local village tours", "Organic food options"]
    },
  ],
  icons: [
    {
      id: 1,
      location: "Delhi, India",
      rating: 4.8,
      distance: "1020 Kilometres away",
      date: "5-10 Mar",
      price: "₹18,500",
      guestFavorite: true,
      images: [
        "/images/packaging/9.jpg",
        "/images/packaging/10.jpg",
        "/images/packaging/11.jpg"
      ],
      description: "Explore the rich history of Delhi with a stay near iconic monuments and heritage sites.",
      amenities: ["Free Breakfast", "WiFi", "City Tour", "Gym"],
      reviews: [
        { user: "Amit", comment: "A must-visit for history lovers!" },
        { user: "Neha", comment: "Great location, close to everything." }
      ],
      highlights: ["Monuments nearby", "City nightlife", "Local street food"]
    },
  ],
  islands: [
    {
      id: 1,
      location: "Goa, India",
      rating: 4.9,
      distance: "600 Kilometres away",
      date: "10-15 Mar",
      price: "₹25,000",
      guestFavorite: true,
      images: [
        "/images/packaging/12.jpg",
        "/images/packaging/13.jpg",
        "/images/packaging/14.jpg"
      ],
      description: "Relax on the pristine beaches of Goa and enjoy island hopping adventures.",
      amenities: ["Beach Access", "Spa", "Restaurant", "Water Sports"],
      reviews: [
        { user: "Priya", comment: "Best beach vacation ever!" },
        { user: "Vikram", comment: "Loved the island hopping experience." }
      ],
      highlights: ["Scuba diving", "Sunset cruises", "Beach parties"]
    },
  ],
  castles: [
    {
      id: 1,
      location: "Rajasthan, India",
      rating: 4.7,
      distance: "800 Kilometres away",
      date: "15-20 Mar",
      price: "₹30,000",
      guestFavorite: true,
      images: [
        "/images/packaging/15.jpg",
        "/images/packaging/16.jpg",
        "/images/packaging/17.jpg"
      ],
      description: "Stay in a majestic castle in Rajasthan and experience royal hospitality.",
      amenities: ["Heritage Stay", "Cultural Shows", "Fine Dining", "Guided Tours"],
      reviews: [
        { user: "Anjali", comment: "Felt like royalty staying here!" },
        { user: "Rohan", comment: "A unique and luxurious experience." }
      ],
      highlights: ["Royal experience", "Cultural performances", "Camel safari"]
    },
  ],
  rooms: [
    {
      id: 1,
      location: "Rajasthan, India",
      rating: 4.7,
      distance: "800 Kilometres away",
      date: "15-20 Mar",
      price: "₹30,000",
      guestFavorite: true,
      images: [
        "/images/packaging/15.jpg",
        "/images/packaging/16.jpg",
        "/images/packaging/17.jpg"
      ],
      description: "Stay in a majestic castle in Rajasthan and experience royal hospitality.",
      amenities: ["Heritage Stay", "Cultural Shows", "Fine Dining", "Guided Tours"],
      reviews: [
        { user: "Anjali", comment: "Felt like royalty staying here!" },
        { user: "Rohan", comment: "A unique and luxurious experience." }
      ],
      highlights: ["Royal experience", "Cultural performances", "Camel safari"]
    },
  ],
  cabins: [
    {
      id: 1,
      location: "Rajasthan, India",
      rating: 4.7,
      distance: "800 Kilometres away",
      date: "15-20 Mar",
      price: "₹30,000",
      guestFavorite: true,
      images: [
        "/images/packaging/15.jpg",
        "/images/packaging/16.jpg",
        "/images/packaging/17.jpg"
      ],
      description: "Stay in a majestic castle in Rajasthan and experience royal hospitality.",
      amenities: ["Heritage Stay", "Cultural Shows", "Fine Dining", "Guided Tours"],
      reviews: [
        { user: "Anjali", comment: "Felt like royalty staying here!" },
        { user: "Rohan", comment: "A unique and luxurious experience." }
      ],
      highlights: ["Royal experience", "Cultural performances", "Camel safari"]
    },
  ],
  topoftheworld: [
    {
      id: 1,
      location: "Rajasthan, India",
      rating: 4.7,
      distance: "800 Kilometres away",
      date: "15-20 Mar",
      price: "₹30,000",
      guestFavorite: true,
      images: [
        "/images/packaging/15.jpg",
        "/images/packaging/16.jpg",
        "/images/packaging/17.jpg"
      ],
      description: "Stay in a majestic castle in Rajasthan and experience royal hospitality.",
      amenities: ["Heritage Stay", "Cultural Shows", "Fine Dining", "Guided Tours"],
      reviews: [
        { user: "Anjali", comment: "Felt like royalty staying here!" },
        { user: "Rohan", comment: "A unique and luxurious experience." }
      ],
      highlights: ["Royal experience", "Cultural performances", "Camel safari"]
    },
  ],
  amazingviews: [
    {
      id: 1,
      location: "Rajasthan, India",
      rating: 4.7,
      distance: "800 Kilometres away",
      date: "15-20 Mar",
      price: "₹30,000",
      guestFavorite: true,
      images: [
        "/images/packaging/15.jpg",
        "/images/packaging/16.jpg",
        "/images/packaging/17.jpg"
      ],
      description: "Stay in a majestic castle in Rajasthan and experience royal hospitality.",
      amenities: ["Heritage Stay", "Cultural Shows", "Fine Dining", "Guided Tours"],
      reviews: [
        { user: "Anjali", comment: "Felt like royalty staying here!" },
        { user: "Rohan", comment: "A unique and luxurious experience." }
      ],
      highlights: ["Royal experience", "Cultural performances", "Camel safari"]
    },
  ],
  farms: [
    {
      id: 1,
      location: "Rajasthan, India",
      rating: 4.7,
      distance: "800 Kilometres away",
      date: "15-20 Mar",
      price: "₹30,000",
      guestFavorite: true,
      images: [
        "/images/packaging/15.jpg",
        "/images/packaging/16.jpg",
        "/images/packaging/17.jpg"
      ],
      description: "Stay in a majestic castle in Rajasthan and experience royal hospitality.",
      amenities: ["Heritage Stay", "Cultural Shows", "Fine Dining", "Guided Tours"],
      reviews: [
        { user: "Anjali", comment: "Felt like royalty staying here!" },
        { user: "Rohan", comment: "A unique and luxurious experience." }
      ],
      highlights: ["Royal experience", "Cultural performances", "Camel safari"]
    },
  ],
  amazingpools: [
    {
      id: 1,
      location: "Rajasthan, India",
      rating: 4.7,
      distance: "800 Kilometres away",
      date: "15-20 Mar",
      price: "₹30,000",
      guestFavorite: true,
      images: [
        "/images/packaging/6.jpg",
        "/images/packaging/16.jpg",
        "/images/packaging/17.jpg"
      ],
      description: "Stay in a majestic castle in Rajasthan and experience royal hospitality.",
      amenities: ["Heritage Stay", "Cultural Shows", "Fine Dining", "Guided Tours"],
      reviews: [
        { user: "Anjali", comment: "Felt like royalty staying here!" },
        { user: "Rohan", comment: "A unique and luxurious experience." }
      ],
      highlights: ["Royal experience", "Cultural performances", "Camel safari"]
    },
  ],
  beachfront: [
    {
      id: 1,
      location: "Rajasthan, India",
      rating: 4.7,
      distance: "800 Kilometres away",
      date: "15-20 Mar",
      price: "₹30,000",
      guestFavorite: true,
      images: [
        "/images/packaging/15.jpg",
        "/images/packaging/16.jpg",
        "/images/packaging/17.jpg"
      ],
      description: "Stay in a majestic castle in Rajasthan and experience royal hospitality.",
      amenities: ["Heritage Stay", "Cultural Shows", "Fine Dining", "Guided Tours"],
      reviews: [
        { user: "Anjali", comment: "Felt like royalty staying here!" },
        { user: "Rohan", comment: "A unique and luxurious experience." }
      ],
      highlights: ["Royal experience", "Cultural performances", "Camel safari"]
    },
  ], omg: [
    {
      id: 1,
      location: "Rajasthan, India",
      rating: 4.7,
      distance: "800 Kilometres away",
      date: "15-20 Mar",
      price: "₹30,000",
      guestFavorite: true,
      images: [
        "/images/packaging/15.jpg",
        "/images/packaging/16.jpg",
        "/images/packaging/17.jpg"
      ],
      description: "Stay in a majestic castle in Rajasthan and experience royal hospitality.",
      amenities: ["Heritage Stay", "Cultural Shows", "Fine Dining", "Guided Tours"],
      reviews: [
        { user: "Anjali", comment: "Felt like royalty staying here!" },
        { user: "Rohan", comment: "A unique and luxurious experience." }
      ],
      highlights: ["Royal experience", "Cultural performances", "Camel safari"]
    },
  ], luxe: [
    {
      id: 1,
      location: "Rajasthan, India",
      rating: 4.7,
      distance: "800 Kilometres away",
      date: "15-20 Mar",
      price: "₹30,000",
      guestFavorite: true,
      images: [
        "/images/packaging/15.jpg",
        "/images/packaging/16.jpg",
        "/images/packaging/17.jpg"
      ],
      description: "Stay in a majestic castle in Rajasthan and experience royal hospitality.",
      amenities: ["Heritage Stay", "Cultural Shows", "Fine Dining", "Guided Tours"],
      reviews: [
        { user: "Anjali", comment: "Felt like royalty staying here!" },
        { user: "Rohan", comment: "A unique and luxurious experience." }
      ],
      highlights: ["Royal experience", "Cultural performances", "Camel safari"]
    },
  ], treehouses: [
    {
      id: 1,
      location: "Rajasthan, India",
      rating: 4.7,
      distance: "800 Kilometres away",
      date: "15-20 Mar",
      price: "₹30,000",
      guestFavorite: true,
      images: [
        "/images/packaging/15.jpg",
        "/images/packaging/16.jpg",
        "/images/packaging/17.jpg"
      ],
      description: "Stay in a majestic castle in Rajasthan and experience royal hospitality.",
      amenities: ["Heritage Stay", "Cultural Shows", "Fine Dining", "Guided Tours"],
      reviews: [
        { user: "Anjali", comment: "Felt like royalty staying here!" },
        { user: "Rohan", comment: "A unique and luxurious experience." }
      ],
      highlights: ["Royal experience", "Cultural performances", "Camel safari"]
    },
  ], lakefront: [
    {
      id: 1,
      location: "Rajasthan, India",
      rating: 4.7,
      distance: "800 Kilometres away",
      date: "15-20 Mar",
      price: "₹30,000",
      guestFavorite: true,
      images: [
        "/images/packaging/15.jpg",
        "/images/packaging/16.jpg",
        "/images/packaging/17.jpg"
      ],
      description: "Stay in a majestic castle in Rajasthan and experience royal hospitality.",
      amenities: ["Heritage Stay", "Cultural Shows", "Fine Dining", "Guided Tours"],
      reviews: [
        { user: "Anjali", comment: "Felt like royalty staying here!" },
        { user: "Rohan", comment: "A unique and luxurious experience." }
      ],
      highlights: ["Royal experience", "Cultural performances", "Camel safari"]
    },
  ], lakefront: [
    {
      id: 1,
      location: "Rajasthan, India",
      rating: 4.7,
      distance: "800 Kilometres away",
      date: "15-20 Mar",
      price: "₹30,000",
      guestFavorite: true,
      images: [
        "/images/packaging/15.jpg",
        "/images/packaging/16.jpg",
        "/images/packaging/17.jpg"
      ],
      description: "Stay in a majestic castle in Rajasthan and experience royal hospitality.",
      amenities: ["Heritage Stay", "Cultural Shows", "Fine Dining", "Guided Tours"],
      reviews: [
        { user: "Anjali", comment: "Felt like royalty staying here!" },
        { user: "Rohan", comment: "A unique and luxurious experience." }
      ],
      highlights: ["Royal experience", "Cultural performances", "Camel safari"]
    },
  ],

};

export const reviewsData = [
  {
    id: 1,
    name: "Niyas",
    date: "May 2023",
    img: "https://d1i3enf1i5tb1f.cloudfront.net/Review/26950/Laura%20R.jpg",
    review:
      "Bird Paradise in Singapore is so beautiful that it is a joy to all ages. The aviaries are well-designed and provide a close encounter with a wide array of colorful and charismatic birds.",
    rating: 5,
    reviewStatus: "Excellent",
  },
  {
    id: 2,
    name: "Roy",
    date: "Jan 2024",
    img: "https://randomuser.me/api/portraits/women/47.jpg",
    review:
      "Absolutely loved my trip to Bali! The beaches, the food, and the people were all amazing. I can’t wait to visit again!",
    rating: 4,
    reviewStatus: "Very Good",
  },
  {
    id: 3,
    name: "Rahul",
    date: "Dec 2023",
    img: "https://randomuser.me/api/portraits/men/32.jpg",
    review:
      "Visiting the Swiss Alps was a dream come true! The snow-covered mountains and peaceful scenery were breathtaking.",
    rating: 5,
    reviewStatus: "Excellent",
  },
  {
    id: 4,
    name: "Samantha",
    date: "Feb 2024",
    img: "https://randomuser.me/api/portraits/women/5.jpg",
    review:
      "I had an amazing experience in Thailand! The street food, the beaches, and the people were all incredible. Highly recommend!",
    rating: 3,
    reviewStatus: "Good",
  },
  {
    id: 5,
    name: "Samantha",
    date: "Feb 2024",
    img: "https://randomuser.me/api/portraits/women/7.jpg",
    review:
      "I had an amazing experience in Thailand! The street food, the beaches, and the people were all incredible. Highly recommend!",
    rating: 3,
    reviewStatus: "Good",
  },
  {
    id: 6,
    name: "Samantha",
    date: "Feb 2024",
    img: "https://randomuser.me/api/portraits/women/8.jpg",
    review:
      "I had an amazing experience in Thailand! The street food, the beaches, and the people were all incredible. Highly recommend!",
    rating: 3,
    reviewStatus: "Good",
  },
  {
    id: 7,
    name: "Samantha",
    date: "Feb 2024",
    img: "https://randomuser.me/api/portraits/women/9.jpg",
    review:
      "I had an amazing experience in Thailand! The street food, the beaches, and the people were all incredible. Highly recommend!",
    rating: 3,
    reviewStatus: "Good",
  },
  {
    id: 8,
    name: "Samantha",
    date: "Feb 2024",
    img: "https://randomuser.me/api/portraits/women/10.jpg",
    review:
      "I had an amazing experience in Thailand! The street food, the beaches, and the people were all incredible. Highly recommend!",
    rating: 3,
    reviewStatus: "Good",
  },
  {
    id: 9,
    name: "Samantha",
    date: "Feb 2024",
    img: "https://randomuser.me/api/portraits/women/11.jpg",
    review:
      "I had an amazing experience in Thailand! The street food, the beaches, and the people were all incredible. Highly recommend!",
    rating: 3,
    reviewStatus: "Good",
  },
  {
    id: 10,
    name: "Samantha",
    date: "Feb 2024",
    img: "https://randomuser.me/api/portraits/women/12.jpg",
    review:
      "I had an amazing experience in Thailand! The street food, the beaches, and the people were all incredible. Highly recommend!",
    rating: 3,
    reviewStatus: "Good",
  },
];

export const priceAndOffersTableData = [
  {
      id: 1,
      tourName: 'Mountain Adventure',
      transfer: 'Without Transfer',
      adult: '0',
      child: '0',
      infant: '0',
      date: '2025-10-10',
      totalAmount: 300,
      selected: false,
      timeSlot: "Select Time Slot",
      fundStatus: "Non Refundable"
  },
  {
      id: 2,
      tourName: 'Dolphinarium  Majestic Swim - Private Session (1 to 3 People)',
      transfer: 'Without Transfer',
      adult: '0',
      child: '0',
      infant: '0',
      date: '2025-10-10',
      totalAmount: 300,
      selected: false,
      timeSlot: "Select Time Slot",
      fundStatus: "Refundable"
  }
];
export const transferTimings = [
  { type: 'Sharing Transfers', timing: 'Pick up at 10:00 AM Drop off at 12:30 PM', duration: '02:00 hours' },
  { type: 'Private Transfers', timing: 'Pick up at 10:00 AM Drop off at 12:30 PM', duration: '2:00 hours' },
  { type: 'Without Transfers', timing: 'Dolphin and Seal show -11am, 02pm and 05pm', duration: '00:45 hours' }
];
export const bookingDetails = [
  { title: 'Voucher Confirmation', subtitle: 'Mobile Voucher', description: 'Use your phone or Print your Voucher' },
  { title: 'Booking Confirmation', subtitle: 'Instant Confirmation', description: 'Instant Tour Confirmation will be Provided' },
  { title: 'Booking Confirmation', subtitle: 'Instant Confirmation', description: 'Instant Tour Confirmation will be Provided' },
  { title: 'Booking Confirmation', subtitle: 'Instant Confirmation', description: 'Instant Tour Confirmation will be Provided' }
];


// export const tourPackages = [
//   {
//     id: 1,
//     title: "Dubai Dolphinarium",
//     pricePerNight: 2500,
//     tourOption: "IMG Entry tickets",
//     date: "8/3/2025",
//     serviceFee: "₹8,894",
//     transferType: "Without Transfers",
//     transferTimings: "Park Timings: Sunday to Friday - 12:00 PM To 10:00 PM Saturday - 12:00 PM To 11:00 PM",
//     lastCancellationDate: "Non Refundable",
//     availability: "Available",
//     amountInclVAT: "AED 225.00",
//     total: "₹71,890",
//   },
//   {
//     id: 2,
//     title: "Empire State Building: General Admission Tickets",
//     pricePerNight: 2500,
//     tourOption: "IMG Entry tickets",
//     date: "8/3/2025",
//     serviceFee: "₹8,894",
//     transferType: "Without Transfers",
//     transferTimings: "Park Timings: Sunday to Friday - 12:00 PM To 10:00 PM Saturday - 12:00 PM To 11:00 PM",
//     lastCancellationDate: "Non Refundable",
//     availability: "Available",
//     amountInclVAT: "AED 225.00",
//     total: "₹71,890",
//   },
//   {
//     id: 3,
//     title: "Statue of Liberty",
//     pricePerNight: 2500,
//     tourOption: "IMG Entry tickets",
//     date: "8/3/2025",
//     serviceFee: "₹8,894",
//     transferType: "Without Transfers",
//     transferTimings: "Park Timings: Sunday to Friday - 12:00 PM To 10:00 PM Saturday - 12:00 PM To 11:00 PM",
//     lastCancellationDate: "Non Refundable",
//     availability: "Available",
//     amountInclVAT: "AED 225.00",
//     total: "₹71,890",
//   },
// ];

export const travelPackages = [
  {
    title: "Dubai Dolphinarium",
    price: "2500",
    details: [
      { label: "Tour Option", value: "IMG Entry tickets" },
      { label: "Date", value: "8/3/2025" },
      { label: "Live-Tour service fee", value: "₹8,894" },
      { label: "Transfer Type", value: "Without Transfers" },
      { label: "Availability", value: "Available" },
      { label: "Amount Incl. VAT", value: "AED 225.00" },
      { label: "Total", value: "₹71,890" },
    ],
    cancelPolicy: "Non Refundable",
  },
  {
    title: "Empire State Building: General Admission Tickets",
    price: "2500",
    details: [
      { label: "Tour Option", value: "General Admission" },
      { label: "Date", value: "8/3/2025" },
      { label: "Live-Tour service fee", value: "₹8,894" },
      { label: "Transfer Type", value: "Without Transfers" },
      { label: "Availability", value: "Available" },
      { label: "Amount Incl. VAT", value: "AED 225.00" },
      { label: "Total", value: "₹71,890" },
    ],
    cancelPolicy: "Non Refundable",
  },
  {
    title: "Statue of Liberty",
    price: "2500",
    details: [
      { label: "Tour Option", value: "Liberty Access Ticket" },
      { label: "Date", value: "8/3/2025" },
      { label: "Live-Tour service fee", value: "₹8,894" },
      { label: "Transfer Type", value: "Without Transfers" },
      { label: "Availability", value: "Available" },
      { label: "Amount Incl. VAT", value: "AED 225.00" },
      { label: "Total", value: "₹71,890" },
    ],
    cancelPolicy: "Non Refundable",
  },
];


export const finalAmount = "₹8,894";
